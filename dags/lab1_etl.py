...
def return_snowflake_conn(autocommit: bool | None = None):
    """Return a Snowflake DB-API connection from Airflow connection."""
    hook = SnowflakeHook(snowflake_conn_id=SNOWFLAKE_CONN_ID)
    conn = hook.get_conn()
    if autocommit is not None:
        # Snowflake connector supports .autocommit attribute
        try:
...
def extract(cfg: Dict) -> List[Dict]:
    ...
    for t in cfg["tickers"]:
        ...
        # 1) Flatten possible MultiIndex columns
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = [c[0] if isinstance(c, tuple) else c for c in df.columns]

        # 2) Upper-case column names
        ...
        out_rows.extend(df[wanted].to_dict(orient="records"))
        logger.info("Extracted %d rows for %s", len(df), t)
...
def transform(raw_records: List[Dict]) -> List[Tuple]:
    ...
    df = pd.DataFrame(raw_records).astype(
        {
            "OPEN": float,
            "HIGH": float,
            "LOW": float,
...
def load(cfg: Dict, tuples: List[Tuple]) -> int:
    ...
    conn = return_snowflake_conn(autocommit=False)
    total = 0
    try:
        with conn.cursor() as cur:
...
with DAG(
    ...
    trigger_train_forecast = TriggerDagRunOperator(
     ...
     # Trigger dbt DAG after all forecasts complete
    trigger_dbt = TriggerDagRunOperator(
        task_id="trigger_dbt_elt",
        trigger_dag_id="BuildELT_Stock_Forecast_dbt",
        wait_for_completion=True,
        allowed_states=[DagRunState.SUCCESS],
        failed_states=[DagRunState.FAILED],
        reset_dag_run=True,
    ...
    loaded >> trigger_train_forecast >> trigger_dbt
